...
```{r warning, echo = FALSE, message = FALSE}
#-------------------------------------------------------#
# RUN THE main.R SCRIPT BEFORE COMPILING THIS DOCUMENT! #
#-------------------------------------------------------#
```
...
project_names <- c("BASE"="Baseline", "PHOS"="Phosphorus", "CSORWM"="CSORWM")
...
# precip
precip <- readRDS("../data/rdata/precip.rda")
wq <- wq %>%
  mutate(Datehour = round_date(Datetime, "hour")) %>%
  left_join(precip, by = c("Datehour"="Datetime"))
...
The Mystic River Watershed Association (MyRWA) has been collecting routine monthly water quality samples along the Malden River since 2000. MyRWA has also collected a significant number of grab samples targeting stormwater outfalls and other locations suspected of high pollutant discharges as part of their Hotspot Monitoring Program. The Massachusetts Water Resources Authority (MWRA) has also collected routine biweekly sampling data on the Malden River since 2002 as part of their CSO Receiving Water Monitoring (CSORWM) Program.

This document provides an exploratory analysis of water quality data collected by both MyRWA and the MWRA on the Malden River from 2000 through 2015. The goals of this analysis are to

1. summarize existing water quality data,
2. provide hypotheses that could potentially explain various patterns that are observed in the data,
2. determine the frequency and magnitude of water quality impairments on the river, and
3. explore whether water quality conditions and impairments have changed over time.

This analysis focuses primarily on two long-term datasets (MyRWA Baseline and MWRA CSORWM Programs), as well as data collected in 2015 as part of MyRWA's new Phosphorus Sampling Program. Although more data are available from individual municipalities and the MyRWA Hotspot Sampling Program, those datasets were excluded from this analysis because they include single grab samples at many individual locations but do not follow regular sampling frequencies. Therefore, those datasets require different kinds of analyses focusing on evaluating the spatial variability of water quality conditions, which is beyond the scope of this analysis.

Finally, it is important to note that this report provides an *exploratory* analysis of water quality data for the Malden River. As an exploratory analysis, the methodology focuses primarily on using data visualization as a means to identify potential patterns and trends in the raw data. The ultimate goal of this effort is to use these observations as a basis for developing testable hypotheses regarding water quality dynamics within the river. However, testing whether those hypotheses are in fact true requires the development and application of statistical and/or simulation models, which was beyond the scope of this study. Furthermore, please note that many of the figures in this report include locally weighted scatterplot smoothing (LOESS) lines that are meant to assist the reader in visually identifying patterns among the clouds of data points. LOESS smoothing lines are a common non-parametric method for identifying non-linear relationships between two variables, and do not require assumptions about the distribution of the data or the shape of the relationship as is the case with regression lines. These smoothing lines are not meant to provide any statistical evidence that a pattern or trend truly exists, but rather to simply serve as visual cues in order to facilitate interpretation and discussion of each figure. 

# Monitoring Stations

Table \ref{tbl-stn} lists the names and coordinates of the three routine sampling stations on the Malden River that are included in this analysis. Two of these stations (`MAR003` and `MAR036`) are sampled by MyRWA, while the third station (`MWRA176`) is sampled by MWRA. Figure 1 shows the locations of these stations as well as the basin boundary and hydrologic features (streams and ponds) in the Malden River watershed. All three monitoring stations are located within the main channel of the Malden River, which is a flat, slow-moving reach impounded by the Amelia Earhart Dam just below its confluence with the Mystic River. Upstream of the main channel, the Malden River flows underground beneath the city of Malden making sampling in reaches just upstream of the main channel very difficult, if not impossible. 
...
## MyRWA Phosphorus Sampling Program

The MyRWA Phosphorus Sampling Program began in 2015 as part of an effort to better understand nutrient loading and in-stream dynamics related to eutrophication of the Mystic River. This program includes biweekly samples collected at both the baseline station (`MAR036`) as well as a new station: `MAR003`. Figure \ref{fig-characteristic-phos} shows which parameters were measured at each site in 2015. Field parameters including dissolved oxygen, specific conductivity, water temperature and turbidity are collected at both stations, as well as total phosphorus. Additionally, chlorophyll a is measured only at the `MAR003` station.

```{r fig-characteristic-phos, fig.width=8, fig.height=4, fig.cap="Water Quality Parameters Measured for the MyRWA Phosphorus Sampling Program by Year \\label{fig-characteristic-phos}"}
wq %>%
  filter(ProjectID == "PHOS") %>%
  mutate(Year = year(Datetime),
         CharacteristicLabel = ordered(CharacteristicLabel, levels = rev(levels(CharacteristicLabel)))) %>%
  select(Year, LocationID, CharacteristicLabel) %>%
  distinct %>%
...
At MyRWA's `MAR036` station, the majority of DO measurements are above the minimum standard of 5 mg/L, except in the most recent year (2015).  Both concentration and percent saturation appear to show a downward trend since 2005 suggesting conditions are becoming more impaired. However, further analysis is needed to verify whether this decreasing trend is indeed significant.

At the MWRA station `MWRA176`, the surface measurements do not suggest any trend and are mostly above the state standard. The measurements collected near the bottom of the water column, however, are primarily below the standard indicating a significant impairment of deeper waters at this location. These low DO levels are likely due to high sediment oxygen demand, which consumes oxygen from the water column as the result of biological and/or chemical processes in the sediment. 
...
The DO percent saturation at `MAR036` shows some seasonal patterns with a greater range of values observed during the summer months (May - September) than in winter or fall. Super-saturated conditions, as indicated by DO saturation above 100%, occasionally occur in late spring and late summer, and may indicate excessive primary production due to the growth of algae or submerged aquatic vegetation which produce oxygen. 

At the MWRA station `MWRA176`, surface DO saturation frequently exceeded 100%, especially in the early summer (May) and late summer (August and September). Because this station is located farther downstream from the MyRWA station, these results may indicate significant algae and/or aquatic vegetation growth occuring between the two stations. The bottom measurements also show significant seasonality with very low concentrations (< 2 mg/L) most frequent in July and August. 

These patterns in DO levels suggest that oxygen levels are frequently impaired near the bottom of the water column, likely due to high sediment oxygen demand, and that super-saturation occurs near the water surface throughout the summer, which suggests high rates of primary production.  
...
Figure \ref{fig-do-yr} shows the summer (May - September) mean DO concentration and saturation levels at each site and for each year. At `MAR036`, summer mean DO concentration and percent saturation both show downward trends since 2004 with the summer mean DO in 2015 just below the standard of 5 mg/L. However, at `MWRA176`, the surface samples are consistently greater than the standard and do not show any major trends suggesting replenishment of oxygen from primary production between the two stations. For the bottom samples at `MWRA176`, there appears to be an increase in summer mean DO concentration and saturation since 2006, however, the summer mean in 2013 was the second lowest over the entire period and thus these trends may not be significant. These results suggest that changes at the top of the channel (`MAR036`) are leading to lower summer dissolved oxygen levels, and will result in frequent DO impairments in the future if this trend continues.

```{r fig-do-yr, fig.width = 10, fig.height = 6, fig.cap="Summer (May-Sept) Mean Dissolved Oxygen Levels and Saturation Levels by Station and Sample Depth \\label{fig-do-yr}"}
wq_lt %>%
  filter(CharacteristicID %in% c("DO", "DO_SAT"),
         month(Datetime) %in% 5:9) %>%
  mutate(Year = year(Datetime)) %>%
  group_by(LocationID, CharacteristicUnits, SampleDepthName, Year) %>%
  summarise(N = n(),
            Mean = mean(ResultValue, na.rm = TRUE)) %>%
  ggplot(aes(Year, Mean, color = SampleDepthName)) +
  geom_hline(data = filter(wq_std, CharacteristicID %in% c("DO", "DO_SAT")),
             aes(yintercept = StdValue, linetype = "WQ Standard"),
             color = "gray50", linetype = 2) +
  geom_point(size = 1) +
...
Temperature is another common water quality parameter measured in the field that is important for supporting certain fish species that can become stressed under extreme conditions. Temperature is also a major controlling factor that affects the rates of biological activity (e.g. algae growth, decomposition) and various chemical processes. The Massachusetts water quality standard for warm water fisheries sets a maximum temperature of 28.3 deg C (83 deg F) year-round. 

Figure \ref{fig-temp-ts} shows temperatures measured at each long term station over time. The horizontal line indicates the state standard of 28.3 deg C. With only a couple exceptions, the temperature measurements at both stations do not exceed the state standard and also do not show any general trend over time. Temperatures at `MWRA176` appear higher than at `MAR036` due to differences in the time of day when samples are collected, and because the MWRA dataset does not include measurements during the winter and spring.
...
Figure \ref{fig-spcond-ts-surface} shows the specific conductivity levels for only the surface measurements at both sites. The trend lines suggest increasing conductivity since 2005 at MAR036, and since 2010 at MWRA176. This trend could be caused by changes in the dam operation or abnormally low precipitation in recent years, which would result in less flushing of the channel by stormwater runoff and thus a greater build up of salty water intruding through the dam. An increase in road salt application during the winter could also cause an increase in specific conductivity at these locations.
...
pH is another important water quality parameter in aquatic systems. As primary producers (e.g. algae and submerged plants) grow, they consume carbon dioxide and thus remove acidity from the water resulting in an increase in pH. As their biomass is decomposed in the fall, carbon dioxide is produced through respiration and decreases pH. Thus, pH can serve as an indicator of eutrophication and excessive algae/plant growth and decay. Extreme high or low pH levels can cause stress to many aquatic organisms, and can also greatly affect biological and chemical processes both in the water column and in the sediment. Most notably, pH is a major controlling factor for the rate of phosphorus release from sediments through its effect on the binding of phosphorus to iron complexes. The release of phosphorus from the sediments is known as internal loading, and can be a significant portion of the total nutrient load to the water column in many systems. The Massachusetts water quality standards include a minimum pH of 6.5 and a maximum of 8.3 for warm water fisheries.

Figure \ref{fig-ph-ts} shows the pH levels near the surface and bottom at the `MWRA176` station over time. pH was only measured in the MyRWA baseline sampling until 2006 when it was discontinued, and thus it is not included here. This figure indicates that pH values near the surface exceeded the state standard maximum of 8.3 in almost every year except 2009. Near the bottom of the water column, pH levels were primarily within the range of the minimum and maximum standards, with only a few exceptions. The elevated pH near the surface likely reflects the effect of primary production on carbonate levels through the release of carbon dioxide. 
...
Secchi depth is a measurement of the transparency of the water column. It is the depth at which an observer at the water surface can no longer see a Secchi disk being lowered into the water column in broad daylight. Higher values indicate clearer water, and lower values indicate greater turbidity, algae abundance and other factors reducing transparency. Because of its simplicity, Secchi depth measurements are commonly used as a proxy for algal biomass as well as the general aesthetics of lakes and reservoirs. It is thus used as an indicator of eutrophication with lower Secchi depth values indicating greater abundance of algae, and thus greater eutrophication.
...
The seasonality of Secchi depths shown in Figure \ref{fig-secchi-jday} indicates low transparency in the early and late summer. In the middle of summer, the average Secchi depth increases to some degree, as well as in the fall. This pattern may reflect two distinct periods of algae growth, one in the early summer and another in late summer/early fall. The later period of growth may be caused by an increase in bio-available phosphorus as algae and submerged aquatic vegetation undergo senescence and decomposition, and/or enrichment of surface waters with phosphorus from the bottom of the water column due to turnover in the fall, but only if the water column is stratified throughout the summer. In addition to algae growth, the change in Secchi depth may also reflect seasonal variation in particulate loading caused by stormwater runoff, which is relatively low in mid-summer due to the warmer weather.
...
Total phosphorus (TP) is a key driver of eutrophication in most freshwater systems because it is often the primary factor limiting algae growth. Excess phosphorus loading can be caused by leakage from sanitary sewers, urban runoff, and release from the sediments themselves. Although there is no numeric standard for phosphorus, the Massachusetts water quality standards include narrative criteria prohibiting eutrophic conditions that can be caused by excessive phosphorus. Numeric criteria for target phosphorus levels are often determined on a site-by-site basis using empirical and/or simulation models. As one point of reference, the USEPA's 1986 Water Quality Criteria ("Gold Book") recommends a maximum TP concentration of 0.025 mg/L in lakes and reservoirs, and 0.050 mg/L in streams. Because the main channel of the Malden River is an impoundment, a target level of 0.025 mg/L is considered appropriate for the purpose of this analysis.
...
E. coli is used as an indicator bacteria species for determining the public health risk of freshwater systems. Elevated E. coli levels indicate possible failing sanitary sewer infrastructure, excess animal waste (especially dogs and birds) associated with urban runoff, and other potential pollution sources. The Massachusetts standard for primary contact recreation (e.g. swimming) includes a single sample maximum of 235 CFU/100mL, and a maximum geometric mean of 126 CFU/100mL. 

```{r wq-ecoli}
wq_ecoli <- wq_lt %>%
  filter(CharacteristicID %in% c("ECOLI")) %>%
  mutate(Weather = ifelse(Precip48 > 0.25, "Wet", "Dry"),
         Weather = ordered(Weather, levels = c("Wet", "Dry")))
```


Figure \ref{fig-ecoli-ts} shows E. coli levels near the water surface at `MAR036` and `MWRA176` over time (no bottom samples are collected for E. coli at `MWRA176`). The individual samples are colored by weather condition based on the following criteria:

- **Wet Weather**: 48-hour antecedent precipitation is 0.25 inches or more
- **Dry Weather**: 48-hour antecedent precipitation is less than 0.25 inches

The antecedent precipitation for each sample was computed from hourly measurements collected at Logan Airport in East Boston, MA (about 3 miles from Malden River).

At both sites, E. coli levels frequently exceed the single sample maximum of 235, with `MAR036` having overall greater levels than `MWRA176`. Both sites also show a rise in concentrations from 2005 to 2012, followed by a decline to 2015.

```{r fig-ecoli-ts, fig.width = 10, fig.height = 4, fig.cap="E. coli Concentrations by Station and Weather Condition over Time \\label{fig-ecoli-ts}"}
wq_ecoli %>%
  ggplot(aes(Datetime, ResultValue)) +
  ...
  scale_color_manual("Weather", values = c("Dry" = "olivedrab3", "Wet" = "steelblue"))
```

Figure \ref{fig-ecoli-exceed} shows the percent of wet and dry weather samples, as well as all samples grouped together, in each year that exceed the single sample maximum of 235 CFU/100mL at both sites. Based on all of the samples, the frequency of exceedence is higher overall at `MAR036` as was seen in the previous figure. In 2010 and 2011, more than 90% of the MyRWA Baseline samples at `MAR036` were in exceedence indicating a significant bacteria impairment. Among the wet weather samples, between 80 to 100% of the samples at `MAR036` in each year were exceedences. Note that in three of the years (2003, 2007 and 2009), no wet weather samples were collected at `MAR036`. At `MWRA176`, the fraction of wet weather exceedences is more variable and ranges from 25 to 100%. Finally, the frequency of dry weather exceedences again tend to be higher at `MAR036` with a maximum of 90% in 2011, although dry weather exceedences also occur at `MWRA176`.

```{r fig-ecoli-exceed, fig.width = 10, fig.height = 6, fig.cap="Percent of Samples Exceeding the Single Sample Maximum for E. coli by Year, Station, and Weather Condition \\label{fig-ecoli-exceed}"}
wq_ecoli_frac_weather <- wq_ecoli %>%
  left_join(select(wq_std, CharacteristicID, StdValue)) %>%
  mutate(Year = year(Datetime)) %>%
  group_by(LocationID, SampleDepthName, Year, Weather) %>%
  summarise(N = sum(!is.na(ResultValue)),
            N_Exceed = sum(ResultValue > StdValue, na.rm=TRUE),
            FracExceed = N_Exceed/N) %>%
  ungroup 
wq_ecoli_frac_all <- wq_ecoli %>%
  left_join(select(wq_std, CharacteristicID, StdValue)) %>%
  mutate(Year = year(Datetime)) %>%
  ...
  summarise(N = sum(!is.na(ResultValue)),
            ...
            FracExceed = N_Exceed/N) %>%
  ungroup %>%
  mutate(Weather = "All Samples")

rbind(wq_ecoli_frac_weather, wq_ecoli_frac_all) %>%
  mutate(Weather = ordered(as.character(Weather), levels = c("All Samples", "Wet", "Dry"))) %>%
  ggplot(aes(factor(Year), FracExceed, fill = Weather)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(Weather ~ LocationID) +
  ...
  scale_fill_manual("Weather", values = c("Dry" = "olivedrab3", "Wet" = "steelblue", "All Samples" = "gray50")) +
...
```{r fig-ecoli-jday, fig.width = 10, fig.height = 4, fig.cap="E. coli Concentrations by Station and Weather Condition over Day of the Year \\label{fig-ecoli-jday}"}
wq_ecoli %>%
  mutate(Jday = yday(Datetime),
         JdayDate = as.Date("2001-01-01") + days(Jday - 1)) %>%
  ggplot(aes(JdayDate, ResultValue)) +
...
Figure \ref{fig-ecoli-precip48} shows the relationship between E. coli levels and 48-hour precipitation at each site. At both sites, E. coli levels tend to increase with increasing antecedent precipitation. At `MAR036`, all but three wet weather samples exceed the SSM standard, as do a large portion of dry weather samples. At `MWRA176`, a greater fraction of both the dry and wet weather samples were below the SSM. However, a significant portion of the wet weather samples still exceeded the SSM, as did all samples with antecedent precipitation of 1 inch or more. These results again indicate that dry weather impairments are common at both sites, but especially at the upstream location `MAR036`, and that both sites receive significant stormwater-driven bacteria loads that cause frequent wet weather impairments.

```{r fig-ecoli-precip48, fig.width = 10, fig.height = 4, fig.cap="E. coli Concentrations vs. 48-hour Antecedent Precipitation \\label{fig-ecoli-precip48}"}
wq_lt %>%
  filter(CharacteristicID %in% c("ECOLI")) %>%
  mutate(Weather = ifelse(Precip48 > 0.25, "Wet", "Dry")) %>%
  ggplot(aes(Precip48, ResultValue)) +
  geom_point(aes(color = Weather), size = 1) +
  geom_smooth(color = "black", se = FALSE, size = 0.5) +
  geom_hline(data = filter(wq_std, CharacteristicID == "ECOLI"),
             aes(yintercept = StdValue, linetype = "WQ Standard"),
  ...
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  facet_grid(. ~ LocationID) +
  labs(x = "48-hour Antecedent Precipitation (inches)", y = "E. coli (CFU or MPN/100mL)") +
  scale_color_manual("Weather", values = c("Dry" = "olivedrab3", "Wet" = "steelblue"))
```


# Phosphorus Sampling Program (2015)

This section focuses on data collected by MyRWA through its new Phosphorus Sampling Program, which began in 2015. The goals of this program are to collect data for 1) estimating phosphorus loads and 2) evaluating the response of receiving water bodies to those loads.

Figure \ref{fig-phos} shows all water quality data collected at both stations in 2015. Chlorophyll a was only measured at `MAR003`, while phosphate was only measured at `MAR036` and never exceeded the detection limit of 0.005 mg/L. This figure shows that DO levels were generally lower at `MAR036`, which is located at the top of the channel. Therefore DO appears to be increasing from that to `MAR003`, which is located at the confluence of the Malden and Mystic Rivers. Conductivity was much greater in the early fall at `MAR003`, which again is closer to the dam and thus more likely to be affected by salt water intrusion. Turbidity levels are fairly similar as are TP levels, although TP shows some differences in the early summary and in October with higher levels at `MAR036`.

The chlorophyll a levels at `MAR003` show two peaks, one in May and another in September indicating two blooms with concentrations of 55.9 and 47.5 ug/L, respectively. While the first bloom reflects the initial growth of algae and aquatic vegetation at the start of the growing season, secondary blooms can occur for a variety of reasons including a sudden increase in biologically-available dissolved phosphorus from decomposing algae/plant matter generatred during the first bloom, as well as turnover of the water column as temperatures become cooler in the fall. This turnover can bring nutrient-rich water from the bottom to the surface, and is often observed in lakes that undergo stratification throughout the summer. 

```{r fig-phos, fig.width = 10, fig.height = 8, fig.cap = "Water Quality Data Collected for MyRWA Phosphorus Sampling Program in 2015 \\label{fig-phos}"}
wq %>%
  filter(ProjectID == "PHOS") %>%
...
```{r phos-means}
phos_means <- wq %>%
  filter(ProjectID == "PHOS", CharacteristicID %in% c("TP", "CHLA")) %>%
...
Together, measurements of chlorophyll a, total phosphorus and Secchi depth can be used to categorize the degree of eutrophication in freshwater systems. The mean chlorophyll a concentration at `MAR003` was `r format(phos_means[which(phos_means$LocationID == "MAR003" & phos_means$CharacteristicID=="CHLA"), "Mean"], digits=3)` ug/L with a maximum of `r format(phos_means[which(phos_means$LocationID == "MAR003" & phos_means$CharacteristicID=="CHLA"), "Max"], digits=3)` ug/L. Overall mean concentrations of TP were `r format(phos_means[which(phos_means$LocationID == "MAR003" && phos_means$CharacteristicID=="TP"), "Mean"], digits=3)` and `r format(phos_means[which(phos_means$LocationID == "MAR036" & phos_means$CharacteristicID=="TP"), "Mean"], digits=2)` mg/L at `MAR003` and `MAR036`, respectively. The mean Secchi depth measured by MWRA at `MWRA176` was `r format(secchi_means$Mean, digits=2)` m in 2015. Based on this evidence, the main channel of the Malden River was eutrophic in 2015 according to the following categories derived from Carlson (1977):
...
____________ | _________________ | ________________ | _______________              
**22.9**	   | **0.032 - 0.036** | **0.92**         | **Malden River**


Lastly, Figure \ref{fig-tp-ts-2015} shows the TP concentrations measured at each MyRWA station in 2015 for both the Baseline and Phosphorus Sampling Programs. While both stations are included in the Phosphorus Program, only `MAR036` is included in the Baseline Program. This figure reveals a major discrepancy between the two sampling programs at `MAR036`: TP concentrations are generally higher for the Baseline program relative to the Phosphorus program. This discrepency is being actively investigated, and the result of differences between the analytical methods used by the two separate labs for the each program. However, data from the Phosphorus program still show that the majority of samples were above the target TP concentration of 0.025 mg/L.

```{r fig-tp-ts-2015, fig.width = 10, fig.height = 4, fig.cap = "Total Phosphorus Measurements in 2015 Comparing MyRWA's Baseline and Phosphorus Sampling Programs \\label{fig-tp-ts-2015}"}
wq %>%
  ...
  labs(x = "Date", y = "TP (mg/L)") +
  ylim(0, NA) +
  scale_color_manual("Project", values = c("Baseline" = "steelblue", "Phosphorus" = "chartreuse3")) +
  theme(axis.text.x = element_text(angle = 30, hjust =1 ))
```
...
The previous sections of this report provide an exploratory analysis of the patterns and dynamics in the Malden River based on each water quality parameter and sampling program. This section provides an overall summary of the major findings and observations from this analysis.
...
Indicator bacteria measurements for E. coli collected by both MyRWA and MWRA reveal frequent impairments under both dry and wet weather based on the state standard for a single sample maximum of 235 CFU/100mL. At the MyRWA site `MAR036`, which is located near the top of the main channel, nearly all wet weather samples exceeded the standard in each year, as did a large portion of dry weather samples. At `MWRA176`, the frequency of impairment was generally lower than at `MAR036`, but still at levels high enough to cause concern. Data from both sites suggest a possible decreasing trend since 2012, however, the significance of this trend has not been thoroughly investigated. The possible causes of dry weather impairments include failing sanitary sewer infrastructure, which can cause untreated sewage to either enter the stormwater sewer system or discharge directly into the river, deposition from birds (e.g. geese and gulls), as well as release from the sediment where bacteria may persist and grow over the summer. Wet weather impairments are more frequent and driven by urban storwmater runoff, which may contain raw sewage from leaky sanitary sewers, and pollutants that are washed off the land surface. Although there are no combined sewer overflows (CSOs) on the Malden River, CSO discharges along Alewife Brook and the Mystic River may also contribute to high bacteria levels if the flow from the Mystic River mixes upstream into the Malden from their confluence.

## Eutrophication

The total phosphorus, chlorophyll a, and Secchi depth measurements collected by MyRWA and MWRA all indicate that the Malden River is in a eutrophic state. In 2015, the overall mean total phosphorus and chlorophyll a at `MAR003` were `r format(phos_means[which(phos_means$LocationID == "MAR003" && phos_means$CharacteristicID=="TP"), "Mean"], digits=3)` mg/L and `r format(phos_means[which(phos_means$LocationID == "MAR003" & phos_means$CharacteristicID=="CHLA"), "Max"], digits=3)` ug/L, respectively, and the mean Secchi depth was `r format(secchi_means$Mean, digits=2)` m at `MWRA176`. These values all fall within the eutrophic range based on the work by Carlson (1977). The chlorophyll a data also show that two blooms occurred in 2015, one in the early summer and one in early fall with peak concentrations of 55.9 and 47.5 ug/L, respectively. 

Furthermore, long-term total phosphorus levels measured by MyRWA at both `MAR036` and `MAR003` generally exceed the recommended target of 0.025 mg/L based on the USEPA's 1986 Water Quality Criteria (Gold Book) for lakes and reservoirs since 2000. Although samples collected as part of the Phosphorus program have lower concentrations than those from the Baseline Monitoring Program due to a difference in the lab analyses, these data also show elevated phosphorus levels, which are likely contributing to high algal biomass. 
...
The Malden River has one of the most highly urbanized watersheds in Massachusetts with a long history of industrial activities and environmental degradation. Routine water quality monitoring data collected by MyRWA and MWRA since 2000 show numerous impairments based on state standards (e.g. DO, pH, and E. coli under both dry and wet weather conditions), and excessive nutrient (phosphorus) levels contributing to eutrophication. These datasets do not show any noticable improvements over recent years, although some parameters show possible trends towards better water quality, the significance of which require further analysis. 

The impoundment of the Malden River's main channel by the Amelia Earhart Dam is likely exacerbating these problems by reducing flushing of the channel, and causing the formation of a salt wedge that reduces mixing of the water column. Improved stormwater management and infrastructure to reduce urban runoff and pollutant loading is needed, as well as further investigations into the sources of pollutants. The history of industrial and heavily urbanized usage of this watershed suggests that legacy pollution stored in the sediments may also be playing a significant role in these impairments. The sediments likely have not only high oxygen demand, but also high phosphorus release rates as well as high concentrations of heavy metals. Further work is needed to evaluate these various hypotheses and determine with greater certainty the causes of water quality degradation in the Malden River in order to develop effective management plans and solutions for addressing these problems.
